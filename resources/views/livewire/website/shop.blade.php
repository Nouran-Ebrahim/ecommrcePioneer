<div>
    <div wire:loading>
        <!-- Content here will be displayed during the request -->
        Loading...
    </div>
    <div class="row g-5">
        <div class="col-lg-3 sidebar-column">
            <div class="sidebar border rounded p-3 bg-light">
                <!-- Categories Filter -->
                <div class="sidebar-section mb-4">
                    <h5 class="fw-bold mb-3">{{ __('dashboard.categories') }}</h5>
                    <ul class="list-unstyled ps-2">
                        @forelse ($categories as $category)
                            <li class="mb-2">
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox" wire:model.live="categoryIds"
                        ...
                        @empty
                            <li class="text-muted">{{ __('website.no_categories') }}</li>
                        @endforelse
                    </ul>
                </div>
                    ...
                    <div class="price-range-container">
                        <div class="mb-3">
                            <div class="d-flex justify-content-between mb-2">
                                <span>Min Price: <strong>{{ $minPrice }} EGP</strong></span>
                                <span>Max Price: <strong>{{ $maxPrice }} EGP</strong></span>
                            </div>
                ...
                <div class="sidebar-section mb-4">
                    ...
                    <ul class="list-unstyled ps-2">
                        @forelse ($brands as $brand)
                            <li class="mb-2">
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox" wire:model.live="brandIds"
                    ...
                    @if (count($categoryIds) > 0 || count($brandIds) > 0 || $minPrice > 0 || $maxPrice < 5000)
                        <div class="active-filters">
                            @if (count($categoryIds) > 0)
                                <div class="mb-2">
                                    <span class="fw-bold d-block mb-1">Categories:</span>
                                        ...
                                        @foreach ($categoryIds as $catId)
                                            ...
                                            @if ($category)
                                                <span class="badge bg-primary">
                                                    {{ $category->name }}
                                                    <button
                                                        wire:click="$set('categoryIds', {{ json_encode(array_values(array_diff($categoryIds, [$catId]))) }})"
                            ...
                            @if (count($brandIds) > 0)
                                <div class="mb-2">
                                    <span class="fw-bold d-block mb-1">Brands:</span>
                                    <div class="d-flex flex-wrap gap-1">
                                        @foreach ($brandIds as $brId)
                                            @php $brand = $brands->firstWhere('id', $brId); @endphp
                                            @if ($brand)
                                                <span class="badge bg-secondary">
                                                    {{ $brand->name }}
                                                    <button
                                                        wire:click="$set('brandIds', {{ json_encode(array_values(array_diff($brandIds, [$brId]))) }})"
                                                        class="btn-close btn-close-white ms-2"
                                                        style="font-size: 0.65rem;"></button>
                                                </span>
                                            @endif
                ...
                <div class="row g-5">
                    <div class="col-lg-12">
                        <div class="product-sorting-section">
                            <div class="result">
                                <p>Showing
                                    <strong>1–{{ $currentProductsCount }}</strong>
                                    of <strong>{{ $totalProducts }}</strong> results
                                </p>
                            </div>
                        </div>
                    </div>
                            ...
                            <div class="col-lg-4 col-sm-6">
                                <div class="product-wrapper">
                                    ...
                                    <div class="product-cart-btn">
                                        <a href="cart.html" class="product-btn">{{ $item->category->name }}</a>
                                    </div>
                                </div>
                            </div>
                        @empty
...
@once
    @push('scripts')
        <script>
            Livewire.on('rerender-component', () => {
                Livewire.components
                    .getComponents()
                    .forEach(component => {
                        if (component.name === 'website.shop') {
                            component.$refresh();
                        }
                    });
            });
