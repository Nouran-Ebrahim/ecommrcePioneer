    ...
    <a href="{{route('website.cart')}}" class="cart-item">
        <span>
            <svg width="35" height="28" viewBox="0 0 35 28" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <text x="26.95" y="11" text-anchor="middle" font-size="10" fill="white" font-family="Arial, sans-serif">
                    {{ $cartItemCount }}
                </text>
            </svg>
        </span>
                ...
                {{-- @if (count($cartItems) > 0) --}}
                    ...
                    <div class="wrapper-item">
                        <div class="wrapper-img">
                            <img src="{{asset('uploads/products/' . $cartItem->product->images()->first()->file_name) }}"
                                alt="img">
                        </div>
                        <div class="wrapper-content">
                            <h5 class="wrapper-title">{{$cartItem->product->name}}</h5>
                            <div class="price">
                                <p class="new-price">{{$cartItem->price}}</p>
                            </div>
                        </div>
                    </div>
                    <a href="javascript:void(0)" wire:click="removeItemFormCart({{$cartItem->id}})" class="close-btn">
